...
def plotting_train_val_loss_accuracy_curves(train_losses, train_accuracies, val_losses, val_accuracies ,num_epochs):
	...
	plt.legend()
	plt.grid('off')
	plt.savefig(f'{imgs_path}/train_val_accuracy_without_regularization.png')
	...
	plt.legend()
	plt.grid('off')
	plt.savefig(f'{imgs_path}/train_val_loss_without_regularization.png')
...
def draw_confusion_matrix(network):
	...
	plt.savefig(f'{confmx_path}/Base_Class_Confusion_Matrix.png')
...
# begin process
	...
	print("\n\n ******TRANING_PROCESS******\n\n")
	train_losses, train_accuracies, val_losses, val_accuracies ,num_epochs = main(num_epochs=35)
